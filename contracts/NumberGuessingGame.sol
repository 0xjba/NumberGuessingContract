...
contract NumberGuessingGame is Ownable {
    ...
    function playGame(
        ...
        if (level == GameLevel.Easy) {
            require(
                (100 * 1e18) >= stake,
                "Stake amount should be between 1 - 100"
            );
            require(
                number >= 1 && number <= 3,
            ...
            require(
                (1000 * 1e18) >= stake && stake >= (100 * 1e18),
                "Stake amount should be between 100 - 1000"
            );
            require(
                number >= 1 && number <= 5,
                "Number should be between 1 and 5"
            );
            ...
            require(
                (100000 * 1e18) >= stake && stake >= (1000 * 1e18),
                "Stake amount should be between 1000 - 100000"
            );
            require(
                number >= 1 && number <= 10,
        ...
        players[msg.sender].consecutiveWins = win
            ? players[msg.sender].consecutiveWins + 1
            : 0;
        players[msg.sender].consecutiveWinsInARow = win
            ? players[msg.sender].consecutiveWinsInARow + 1
            : 0;
        ...
        if (win) {
            ...
            players[msg.sender].balance -= stake;
        }
    ...
    function calculateMediumReward(
        uint256 stake,
        bool win
    ) internal view returns (uint256) {
    ...
    function calculateDifficultReward(
        uint256 stake,
        bool win
    ) internal view returns (uint256) {
        ...
        if (win) {
            ...
            if (players[msg.sender].consecutiveWinsInARow == 1) {
                ...
                uint256 currentBonus = initialBonusPercentage +
                    (players[msg.sender].consecutiveWinsInARow - 1) *
                    bonusIncrementPercentage;
