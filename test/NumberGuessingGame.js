...
describe('NumberGuessingGame', () => {
  ...
  beforeEach(async () => {
    ...
    token = await Token.deploy('MyToken', 'WETH', ownerAddess, { gasLimit: 10548748 });
    tokenAddress = await token.getAddress();

    NumberGuessingGame = await ethers.getContractFactory('NumberGuessingGame');
    numberGuessingGame = await NumberGuessingGame.deploy(tokenAddress, ownerAddess, { gasLimit: 10548748 });
    contractAddress = await numberGuessingGame.getAddress();
  ...
  it('should allow the player to play the game', async () => {
    ...
    console.log(playerData.balance, STAKE_AMOUNT);
    expect(playerData.balance).to.equal(ethers.parseEther('60'));
    expect(playerData.consecutiveWins).to.equal(1);
  });
